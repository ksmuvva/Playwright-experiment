    ...
    async def playwright_evaluate(self, script: str, page_index: int = 0, arg: Any = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if "() =>" not in script and "function" not in script:
                # Handle scripts that already have return statements
                if script.strip().startswith("return "):
                    # Remove the leading return keyword to avoid duplicate returns
                    clean_script = script.strip()[7:].strip()
                    logger.warning(f"Script starts with 'return', removing it before wrapping: {script}")
                    wrapped_script = f"() => {{ return {clean_script} }}"
                    logger.info(f"Wrapped script in function with single return: {wrapped_script}")
                    result = await page.evaluate(wrapped_script, arg)
                # For scripts with array operations like map() that may need explicit return
                elif "map(" in script or "filter(" in script or "reduce(" in script or "forEach(" in script:
                    # Need to explicitly return the result of these operations
                    if "return " in script and not script.strip().startswith("return "):
                        logger.warning(f"Script contains a return statement inside a callback: {script}")
                        # Handle case where return is in the middle (in a callback)
                        wrapped_script = f"() => {{ {script} }}"
                    else:
                        logger.warning(f"Script contains array operations that may need explicit return: {script}")
                        wrapped_script = f"() => {{ return {script} }}"
                    logger.info(f"Wrapped script in function: {wrapped_script}")
                    result = await page.evaluate(wrapped_script, arg)
                else:
                    # Regular expressions - just wrap in a function
                    if "return " in script and not script.strip().startswith("return "):
                        ...
                        # Simple expression that doesn't need explicit return
                        result = await page.evaluate(script, arg)
            else:
                result = await page.evaluate(script, arg)
            
            # For debugging: log the type and sample of the result
            result_type = type(result).__name__
            result_sample = str(result)[:100] + "..." if len(str(result)) > 100 else str(result)
            logger.info(f"Evaluation result type: {result_type}, sample: {result_sample}")
            
            return {
                "status": "success",
                "result": result,
                "message": f"Script executed successfully, returned {result_type}"
            }
            
        except Exception as e:
            error_msg = str(e)
            logger.error(f"Error evaluating script: {error_msg}")
            ...
            if "Illegal return statement" in error_msg:
                ...
                error_msg += "\nA variable or function is undefined. Check if all objects exist on the page."
            elif "null" in error_msg.lower():
                error_msg += "\nAn object is null. Check if all elements exist on the page before accessing them."
            elif "not a function" in error_msg.lower():
                error_msg += "\nTried to call something that's not a function. Check method names and object properties."
            
            # Try to offer a corrected script as suggestion
            corrected_script = None
            if "map" in script and "return" in script:
                # Missing return for array operation
                if script.strip().startswith("return "):
                    # Remove the leading return to avoid duplicate returns
                    clean_script = script.strip()[7:].strip()
                    corrected_script = f"() => {{ return {clean_script} }}"
                else:
                    corrected_script = f"() => {{ return {script} }}"
                logger.info(f"Suggested correction: {corrected_script}")
            
            return {
                "status": "error", 
                "message": error_msg,
                "script": script,  # Include the problematic script for debugging
                "corrected_script": corrected_script  # Include a suggested correction if available
            }
